...
if page == "📈 Top Picks":
    ...
    if top_picks_live:
        ...
        # Create tabs for Top Picks details and ML predictions
        detail_tab, ml_tab, all_ml_tab = st.tabs(["📊 Top Picks Details", "🤖 ML Predictions", "🌐 All Tickers ML"])
        ...
        with ml_tab:
            ...
            for pick in top_picks_live:
                ...
                if ml_pred:
                    ...
                    ml_data.append({
                        ...
                        "Signal": ml_pred.get("signal", "N/A"),
                        "Confidence": ml_pred.get("confidence", "N/A"),
                        "Pred Return (1d)": f"{ml_pred.get('pred_return', 0):+.2f}%",
                        "1d": f"{multiday.get('1d', 0):+.2f}%",
                        "3d": f"{multiday.get('3d', 0):+.2f}%",
                        "5d": f"{multiday.get('5d', 0):+.2f}%",
                        "7d": f"{multiday.get('7d', 0):+.2f}%",
                    })
            ...
            if ml_data:
                ...
                st.info("Tidak ada ML predictions tersedia untuk tickers yang dipilih.")


        with all_ml_tab:
            st.subheader("🌐 All Tickers ML Predictions")
            import pandas as pd

            ml_data_all = []
            composites = last_result.get("composites", {})
            for ticker, comp in composites.items():
                ml_pred_all = comp.get("ml_prediction") or {}
                if ml_pred_all:
                    multiday = ml_pred_all.get("predictions_multiday") or {}
                    ml_data_all.append({
                        "Ticker": ticker,
                        "Signal": ml_pred_all.get("signal", "N/A"),
                        "Confidence": ml_pred_all.get("confidence", "N/A"),
                        "Pred Return (1d)": f"{float(ml_pred_all.get('pred_return', 0)):+.2f}%",
                        "1d": f"{float(multiday.get('1d', 0)):+.2f}%",
                        "3d": f"{float(multiday.get('3d', 0)):+.2f}%",
                        "5d": f"{float(multiday.get('5d', 0)):+.2f}%",
                        "7d": f"{float(multiday.get('7d', 0)):+.2f}%",
                    })

            if ml_data_all:
                df_ml_all = pd.DataFrame(ml_data_all)
                st.dataframe(df_ml_all, use_container_width=True, hide_index=True)
            else:
                st.info("Tidak ada ML predictions tersedia untuk semua tickers.")
    elif signals:
        # Show from database
        run_date = signals[0]["run_date"]
        st.caption(f"Data from: {run_date}")

        detail_tab_db, ml_tab_db, all_ml_tab_db = st.tabs(["📊 Top Picks Details", "🤖 ML Predictions", "🌐 All Tickers ML"])
        ...
        with ml_tab_db:
            ...
            for sig in signals:
                ...
                if ml_pred:
                    ...
                    ml_data_db.append({
                        ...
                        "Signal": ml_pred.get("signal", "N/A"),
                        "Confidence": ml_pred.get("confidence", "N/A"),
                        "Pred Return (1d)": f"{float(ml_pred.get('pred_return', 0)):+.2f}%",
                        "1d": f"{float(multiday.get('1d', 0)):+.2f}%",
                        "3d": f"{float(multiday.get('3d', 0)):+.2f}%",
                        "5d": f"{float(multiday.get('5d', 0)):+.2f}%",
                        "7d": f"{float(multiday.get('7d', 0)):+.2f}%",
            ...
            if ml_data_db:
                df_ml_db = pd.DataFrame(ml_data_db)
                st.dataframe(df_ml_db, use_container_width=True, hide_index=True)
            else:
                st.info("Tidak ada ML predictions tersedia untuk tickers yang dipilih.")
        ...
        with all_ml_tab_db:
            ...
            all_signals_db = query_db(f"""
                SELECT ticker, ml_prediction FROM signals
                WHERE run_date = '{run_date}'
            """)

            ml_data_all_db = []
            for sig_all in all_signals_db:
                ml_pred_all = sig_all.get("ml_prediction") or {}
                if ml_pred_all:
                    multiday = ml_pred_all.get("predictions_multiday") or {}
                    ml_data_all_db.append({
                        "Ticker": sig_all.get("ticker", ""),
                        "Signal": ml_pred_all.get("signal", "N/A"),
                        "Confidence": ml_pred_all.get("confidence", "N/A"),
                        "Pred Return (1d)": f"{float(ml_pred_all.get('pred_return', 0)):+.2f}%",
                        "1d": f"{float(multiday.get('1d', 0)):+.2f}%",
            ...
            if ml_data_all_db:
                df_ml_all_db = pd.DataFrame(ml_data_all_db)
                st.dataframe(df_ml_all_db, use_container_width=True, hide_index=True)
            else:
                st.info("Tidak ada ML predictions tersedia untuk semua tickers.")
    else:
